...
async def fill_admin_state(message: types.Message, session: AsyncSession, state: FSMContext):
    try:
        ...
        if media_group:
            await message.bot.send_media_group(548349299, media=media_group)
        if video_content:
            for index, video_id in enumerate(video_content):
                if index == len(video_content) - 1:
                    await message.bot.send_video(548349299, video=video_id, reply_markup=get_inline(callback_data=callback))
                else:
                    await message.bot.send_video(548349299, video=video_id)
